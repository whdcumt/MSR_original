...
int *GetExpandPos(int Length, int Left, int Right, int Edge)
{
	...
	int*Pos = (int*)malloc((Left + Length + Right) * sizeof(int));
...
*/  
int flag=0;
double * BoxBlur(double *Src, double *Dest,int Width, int Height,int Channel, int Radius, int Edge)  /*(1)void __stdcall 改为double.(2)TMatrix *Src修改为double *src.(3)TMatrix *Dest修改为double *Dest(4)EdgeMode Edge修改int Edge(5)增加参数:图像的高和宽,int width, int height**/
{  
    int X, Y, Z, Index;  /*X,Y,Z循环自变量,Width为图像宽度,Height为图像高度,Channel为通道数,Index未知,(2删除Width和Height,Channel)*/
    double Value, ValueB, ValueG, ValueR;           /*(1)ValueB,ValueG,ValueR为三个通道的数值,(2)数据类型由int修改为double,Value值未知*/ 
    int *RowPos, *ColPos;/*横向和纵向扩展后的坐标*/
    double *ColSum, *Diff;       
  

    int Size = 2 * Radius + 1; /*模糊窗的长或宽度*/
    int Amount = Size * Size;/*模糊窗的大小,或者说总像素数*/
    int HalfAmount = Amount / 2;  /*模糊窗大小的一半*/
  
    RowPos = GetExpandPos(Width, Radius, Radius, Edge);  /*按照制定的边缘方式扩展各像素的坐标.这个是width的扩展*/
    ColPos = GetExpandPos(Height, Radius, Radius, Edge); /*按照制定的边缘方式扩展各像素的坐标.这个是Height的扩展*/  
    ColSum = (double *)malloc(Width * sizeof(double)); /* (1)AllocMemory()开辟内存方式修改为malloc()方式(2)int修改为double,创建列空间*/
    Diff   = (double *)malloc((Width - 1) * sizeof(double));  /* (1)AllocMemory()开辟内存方式修改为malloc()方式(2)int修改为double,像素偏移一列*/
    double *RowData = (double *)malloc(Width + 2 * Radius);  /*(1)unsigned char修改为double(2)AllocMemory()开辟内存方式修改为malloc()方式*/
    double *Data = (double *)malloc(Height * Width *sizeof(double));/*开辟内存空间存储临时图像数据 Data对应着原来的SUM->data*/
    for (Y = 0; Y < Height; Y++)                 /* 水平方向的耗时比垂直方向上的大 */ 
    {  
        double *LinePS = Src + Y * Width*sizeof(double);       /*(1) Src->WidthStep一行数据占的字节数,修改为Width*Channel*sizeof(double)*/
        double *LinePD    = (double *)(Data + Y *Width*sizeof(double)); /*(1)Sum->Data修改为Data(2)Sum->WidthStep修改为Width*Channel*sizeof(double)*/  
       /* 拷贝一行数据及边缘部分部分到临时的缓冲区中*/
            ...
            RowData[X] = LinePS[RowPos[X]];                               
            double *AddPos = RowData + Size;  /*unsigned char修改为double*/
            double *SubPos = RowData;                   /*unsigned char修改为double*/
  ...
  /****************************************************************************************/  
    ...
    {  
        double *LinePS = (double *)(Data + ColPos[Y] * Width*sizeof(double));  /*(1)int修改为double(2)Sum->Data修改为Data(3)Sum->WidthStep修改为Width*Channel*sizeof(double)*/
        for(X = 0; X < Width; X++) ColSum[X] += LinePS[X];  
    }  
  
    for (Y = 0; Y < Height; Y++)  
    {  
        double* LinePD  = Dest + Y * Width*sizeof(double);    /*(1)unsigned char修改为double(2)Dest->Data修改为Dest(3)Dest->WidthStep修改为Width*Channel*sizeof(double)*/ 
        double *AddPos     = (double*)(Data + ColPos[Y + Size - 1] *Width*sizeof(double));/*(1)int修改为double(2)Sum->data修改为Data(3)Dest->WidthStep修改为Width*Channel*sizeof(double)*/
        double *SubPos     = (double*)(Data + ColPos[Y] * Width*sizeof(double));/*(1)int修改为double(2)Sum->data修改为Data(3)Sum->WidthStep修改为Width*Channel*sizeof(double) */
        for(X = 0; X < Width; X++)  
        {  
...
>>>>>>> 0d332d7f530e3702ec096ef110976a0de7e4e553
/**
